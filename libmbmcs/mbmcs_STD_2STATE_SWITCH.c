...
/**
 * @file mbmcs_STD_2STATE_SWITCH.c
 * @brief 標準的な２ステートスイッチの切り替え取得オブジェクトライブラリ
 **/
...
/**
 * @fn int mbmcs_std_2state_switch_init(mbmcs_std_2state_switch_t * const self_p, const uintptr_t base_addr)
 * @brief mbmcs_std_2state_switch_tインスタンスを初期化します。
 * @param self_p　mbmcs_std_2state_switch_tインスタンスポインタ
...
/** 
 * @fn void mbmcs_std_2state_switch_sens_negative_enable( mbmcs_std_2state_switch_t * const self_p, const uint8_t is_true)
 * @brief スイッチのネガティブエッジ検出動作の有効・無効を設定します
 * @param self_p　mbmcs_std_2state_switch_tインスタンスポインタ
 * @param is_on 0:無効 1:有効
 */ 
...
/** 
 * @en void mbmcs_std_2state_switch_sens_positive_enable( mbmcs_std_2state_switch_t * const self_p, const uint8_t is_on)
 * @brief スイッチのポジティブエッジ検出動作の有効・無効を設定します
 * @param self_p　mbmcs_std_2state_switch_tインスタンスポインタ
 * @param is_on 0:無効 1:有効
 */ 
...
/**
 * @fn void mbmcs_std_2state_switch_irq_enable( mbmcs_std_2state_switch_t * const self_p, const uint8_t is_on)
 * @brief エッジ検出時の割り込みの有効・無効を設定します。
 * @param self_p　mbmcs_std_2state_switch_tインスタンスポインタ
 * @param is_on 0:無効 1:有効
 */
...
/**
 * @fn int mbmcs_std_2state_switch_get_flags( mbmcs_std_2state_switch_t * const self_p, mbmcs_std_2state_switch_status_t * const stat_p);
 * @brief スイッチの状態を取得します
 * @param self_p　mbmcs_std_2state_switch_tインスタンスポインタ
 * @param stat_p mbmcs_std_2state_switch_status_t構造体ポインタ
 * @retval EINVAL 引数が不正
 * @retval 0 成功
 **/
int mbmcs_std_2state_switch_get_flags(const mbmcs_std_2state_switch_t * const self_p, mbmcs_std_2state_switch_status_t * const stat_p)
{
	...
	// xil_printf("mbmcs_std_2state_switch_get_flags : reg32(mask)=%08x" EOL_CRLF,
	// (reg32 & (uint32_t)SW_CURRENT_LEVEL_MASK));
#if 0
...
/**
 * @fn void mbmcs_std_2state_switch_clear_detective_egde_flags( mbmcs_std_2state_switch_t * const self_p)
 * @brief エッジ検出のビットフラグをクリアします。
 * @param self_p　mbmcs_std_2state_switch_tインスタンスポインタ
 **/ 
...
/**
 * @fn uint32_t _mbmcs_std_2state_switch_get_registers( mbmcs_std_2state_switch_t * const self_p)
 * @brief 生のレジスタ値を取得する
 * @param self_p　mbmcs_std_2state_switch_tインスタンスポインタ
 * @return 32ビットレジスタ値
 **/
...
/**
 * @fn void _mbmcs_std_2state_switch_dump_registers( mbmcs_std_2state_switch_t * const self_p)
 * @brief レジスタ値をダンプします。
 **/
