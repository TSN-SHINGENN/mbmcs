 ...
 *
 *	Basic Author: Seiichi Takeda  '2015-August-15 Active
...
/* ADDR OFFSET:0x8 */
#define ATMEL93C_READMASK 0xffff /* ROMに従う */
/* ADDR OFFSET:0xc */
#define ATMEL93C_WRITEMASK 0xffff /* ROMに従う */
...
/**
 * @fn int mbmcs_atmel_93c_init( mbmcs_atmel_93c_spi_t * const self_p, intptr_t base_addr, enum_mbmcs_atmel_93c_series_t model)
 * @brief オブジェクトの初期化をします
 * @param self_p オブジェクトインスタンスポインタ
 * @param base_addr オブジェクトにマップされたロジックオブジェクトのベースアドレス
 * @param model 外付けされたATMEL製EEPROM型名指定
 * @retval 0 成功
 * @retval ENOTSUP サポート外（又はコーディングされていない）IC
 * @retval ENXIO ロジックオブジェクトとアドレスのビット深度が合わない
 */
int mbmcs_atmel_93c_init( mbmcs_atmel_93c_spi_t * const self_p, const intptr_t base_addr, const enum_mbmcs_atmel_93c_series_t model)
{
    ...
    /* ベースアドレスからステータスからbit-depthとデータのビット深度を確認する */
...
/**
 * @fn int mbmcs_atmel_93c_set_vcc_attribute( mbmcd_atmel_93c_spi_t * const self_p, const uint8_t vcc_is_over_4p5)
 * @brief EEPROMのVCCに4.5V以上の電圧がかかっていることを宣言します。
 *	フラグを立てた場合に、一部の機能が使用可能になります。
 * @param self_p オブジェクトインスタンスポインタ
 * @param vcc_is_over_4p5 1以上：有効 0:無効
 * @retval 0固定 成功
 **/
...
#if 0
/**
 * @fn int mbmcs_atmel_93c_set_interrupt_callback( mbmcs_atmel_93c_spi_t * const self_p, void (*func)(mbmcs_atmel_93c_intr_t*))
 * @brief 割り込み処理を有効にします。
 *     コールバック関数では割り込みレジスタを確認して有効だったら１を無効なら0を戻します。
 * @param self_p オブジェクトインスタンスポインタ
 * @param func 割り込み確認用、コールバック関数(NULLはポーリング制御)
 * @retval 0固定 成功
 **/
...
int mbmcs_atmel_93c_readx8( const mbmcs_atmel_93c_spi_t * const self_p, const uint16_t addr, uint8_t *const d8_p)
{
    ...
    reg32 &= ~(OP_OPCODE_MASK | OP_SUBOP_MASK); /* opcode 0 */
...
int mbmcs_atmel_93c_writex8( const mbmcs_atmel_93c_spi_t * const self_p, const uint16_t addr, const uint8_t d8)
{
    ...
    reg32 &= ~(OP_OPCODE_MASK | OP_SUBOP_MASK); /* op:0x1 subop:0x0 */
...
int mbmcs_atmel_93c_erasex8( const mbmcs_atmel_93c_spi_t * const self_p, const uint16_t addr)
{
    ...
    reg32 |= ((OP_OPCODE_BIT & 0x3) << OP_OPCODE_SHIFT); /* op:0x3 subop:0 */
    reg32 |= OP_START_MASK;
...
int mbmcs_atmel_93c_write_disable( const mbmcs_atmel_93c_spi_t * const self_p)
{
    ...
    reg32 &= ~(OP_OPCODE_MASK | OP_SUBOP_MASK); /* op:0x0 subop:0x0 */
    reg32 |= OP_START_MASK;
